import React from "react";
import PropTypes from "prop-types";
import styled from "styled-components";
import Loader from "../../Components/Loader";
import Helmet from "react-helmet";

const Container = styled.div`
  height: calc(100vh - 50px);
  width: 100%;
  position: relative;
  padding: 50px;
`;

const Backdrop = styled.div`
  position: absolute;
  top: 0;
  left: 0;
  width: 100%;
  height: 100%;
  background-image: url(${props => props.bgImage});
  background-position: center center;
  background-size: cover;
  filter: blur(3px);
  opacity: 0.5;
  z-index: 0;
`;

const Content = styled.div`
  display: flex;
  width: 100%;
  height: 100%;
  position: relative;
  z-index: 1;
`;
const Cover = styled.div`
  width: 30%;
  background-image: url(${props => props.bgImage});
  background-position: center center;
  background-size: cover;
  /* height: 100%; */
  border-radius: 5px;
`;

const Data = styled.div`
  width: 70%;
  margin-left: 10px;
`;

const Title = styled.h3`
  font-size: 32px;
  margin-bottom: 10px;
`;

const ItemContainer = styled.div`
  margin: 20px 0;
  font-size: 17px;
`;

const Item = styled.span``;

const Divider = styled.span`
  margin: 0 10px;
`;

const Overview = styled.p`
  margin-top: 10px;
  font-size: 14px;
  opacity: 0.7;
  line-height: 1.5;
  width: 70%;
`;

const ImdbLink = styled.a``;
const ImdbIcon = styled.img`
  width: 40px;
  height: 20px;
  vertical-align: middle;
`;

const Production = styled.div`
  display: flex;
  margin-top: 5px;
`;

const Companies = styled.div`
  float: left;
  width: 35%;
`;
const Countries = styled.div`
  float: left;
  width: 35%;
`;
const ProductionContent = styled.p`
  font-size: 12px;
  margin: 5px 0px;
  opacity: 0.9;
`;

const YoutubeBox = styled.div`
  width: 70%;
`;
const YoutubeBoxIframe = styled.iframe`
  width: 40%;
  height: 20%;
  margin: 7px;
`;

const DetailPresenter = ({ result, loading, error }) =>
  loading ? (
    <>
      <Helmet>
        <title>Loading | FilmMate</title>
      </Helmet>
      <Loader />
    </>
  ) : (
    <Container>
      <Helmet>
        <title>
          {result.original_title ? result.original_title : result.original_name}{" "}
          | FilmMate
        </title>
      </Helmet>
      <Backdrop
        bgImage={`https://image.tmdb.org/t/p/original${result.backdrop_path}`}
      />
      <Content>
        <Cover
          bgImage={
            result.poster_path
              ? `https://image.tmdb.org/t/p/original${result.poster_path}`
              : require("../../assets/noPosterSmall.png")
          }
        />
        <Data>
          <Title>
            {result.original_title
              ? result.original_title
              : result.original_name}
          </Title>
          <ItemContainer>
            <Item>
              {result.release_date
                ? result.release_date
                  ? result.release_date.substring(0, 4)
                  : result.first_air_date.substring(0, 4)
                : "2019"}
            </Item>
            <Divider>◆</Divider>
            <Item>
              {result.runtime
                ? (result.runtime
                    ? result.runtime
                    : result.episode_run_time[0]) + " min"
                : "No Data"}
            </Item>
            <Divider>◆</Divider>
            <Item>
              {result.genres &&
                result.genres.map((genre, index) =>
                  index === result.genres.length - 1
                    ? genre.name
                    : `${genre.name} / `
                )}
            </Item>
            <Divider>◆</Divider>
            <Item>
              {result.imdb_id ? (
                <ImdbLink href={`https://www.imdb.com/title/${result.imdb_id}`}>
                  <ImdbIcon src={require("../../assets/imdbIcon.png")} />
                </ImdbLink>
              ) : (
                `No Imdb`
              )}
            </Item>
            <Production>
              <Companies>
                Companies
                {result.production_companies &&
                  result.production_companies.map(companies => {
                    return (
                      <>
                        <ProductionContent>
                          ・ {companies.name}
                        </ProductionContent>
                      </>
                    );
                  })}
              </Companies>

              <Countries>
                Countries
                {result.production_countries &&
                  result.production_countries.map(countries => {
                    return (
                      <ProductionContent>・ {countries.name}</ProductionContent>
                    );
                  })}
              </Countries>
            </Production>
            <Overview>{result.overview}</Overview>
            <YoutubeBox>
              {result.videos.results &&
                result.videos.results.map(result => (
                  <YoutubeBoxIframe
                    src={`https://www.youtube.com/embed/${result.key}`}
                  />
                ))}
            </YoutubeBox>
          </ItemContainer>
        </Data>
      </Content>
    </Container>
  );

DetailPresenter.propTypes = {
  result: PropTypes.object,
  loading: PropTypes.bool.isRequired,
  error: PropTypes.string
};

export default DetailPresenter;
